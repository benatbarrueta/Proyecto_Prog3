	...
	public VentanaAsignatura(Object objeto, String tipo, Asignatura asignatura) {
		...
		if (tipo.equals("Alumno")) {
			...
			modeloTareaLista = new DefaultTableModel(
					new Object[] { "NOMBRE", "FECHA ENTREGA", "ESTATUS", "CALIFICACION" }, 0);
			...
			for (Tarea tarea : listaTarea) {
				...
				modeloTareaLista.addRow(new Object[] { tarea.getNombre(), tarea.getFecha_fin(), status,
						tarea.getCalificacion(), alumno.getNombre() });
			}
			...
			modeloTareaLista = new DefaultTableModel(
					new Object[] { "NOMBRE TAREA", "FECHA ENTREGA", "NOMBRE ALUMNO", "CALIFICACION" }, 0);
			tareaLista = new JTable(modeloTareaLista);
			...
			for (Tarea s : listaTarea) {
				...
				for (Alumno a : GestorBD.gestorBD.obtenerDatosAlumnos()) {
					...
					if (a.getId() == s.getId_alumna()) {
						...
						modeloTareaLista.addRow(
								new Object[] { s.getNombre(), s.getFecha_fin(), a.getNombre(), s.getCalificacion() });
					}
						...
						if (tarea.getNombre().equals(nombreTarea)) {
							...
							for (Alumno a : GestorBD.gestorBD.obtenerDatosAlumnos()) {
								if (a.getId() == tarea.getId_alumna()) {
									...
									modeloTareaLista.addRow(new Object[] { tarea.getNombre(), tarea.getFecha_fin(),
											a.getNombre(), tarea.getCalificacion() });
								}
							}
		...
		for (int c = 0; c < tareaLista.getColumnCount(); c++) {
			Class<?> col_class = tareaLista.getColumnClass(c);
